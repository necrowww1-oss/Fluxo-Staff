...
/* ── DASHBOARD ── */
function Dashboard({ events, buyers, onEventClick, onCreateEvent }) {
    ...
    <div className="screen">
      ...
      <div className="stat-grid">
        ...
        <div className="stat-card">
          ...
          <div className="stat-val mono">{buyers.filter(g => g.checkedIn).length}</div>
...
function Sales({ events }) {
  ...
  // Sincronizar activeEvent cuando llegan los eventos de Firebase
  useEffect(() => {
    if (!activeEvent && events.length > 0) setActiveEvent(events[0]);
  }, [events]);

  if (!events.length || !ev) return (
    <div className="screen">
      <div className="topbar"><div className="topbar-title">Ventas</div></div>
      <div className="empty"><div className="empty-ico">📊</div>Cargando eventos…</div>
    </div>
  );

  const revenue  = ev.types?.reduce((s, t) => s + t.sold * t.price, 0) ?? 0;
  const sold     = ev.types?.reduce((s, t) => s + t.sold, 0) ?? 0;
  const capacity = ev.types?.reduce((s, t) => s + t.total, 0) ?? 0;
...
function Guests({ guests, guestlist, events }) {
...
function Scanner({ buyers }) {
  ...
  const mockCodes = buyers.filter(g => !g.checkedIn).slice(0, 3).map(g => g.hash);
...
function EventForm({ form, setForm, onSubmit, onCancel, submitLabel }) {
  ...
  function handlePhoto(e) {
    ...
    reader.onload = ev => {
      // Comprimir a max 400px y calidad 0.5 para que quepa en Firestore (<1MB)
      const img = new Image();
      img.onload = () => {
        const MAX = 400;
        ...
        setF("photo", canvas.toDataURL("image/jpeg", 0.5));
      };
  ...
  return (
    <>
      ...
      <div style={{ background: "var(--bg)", minHeight: "100vh", maxWidth: 430, margin: "0 auto", position: "relative" }}>
        ...
        {tab === "guests"  && <Guests guests={buyers} guestlist={guestlist} events={events} />}
        {tab === "scanner" && <Scanner buyers={buyers} />}
